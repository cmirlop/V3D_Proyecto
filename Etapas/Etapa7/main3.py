...
def getWindowCentered(y, x, img, offset=0): 
    ...
    x_start, x_end = x - block_half - offset + 1, x + block_half - offset + 1
...
def getDisparityMap(left, right):
    ...
    for y in range(block_half, h - block_half):      
        for x in range(max_disp, w - block_half):
            errors = np.full(max_disp, np.inf, dtype=np.float32)  # Inicializa errores con infinito
            
            # Calcula la ventana en una sola operación
            ...
            for offset in range(max_disp):               
                ...
                if W_left.shape == W_right.shape:
                    errors[offset] = np.sum((W_left - W_right) ** 2)

            best_offset = np.argmin(errors)  # Encuentra el índice con menor error
...
def reproject_image_to_3D(disparity, T_1):
    ...
    for y in range(height):
        ...
        sys.stdout.write(f"{msg}")
        sys.stdout.flush()
    #print(f"{point[:3]} - {point[3]} - {vec} - {Q} - {points_3D[height-1, width-1]} - {point}")
    return points_3D
...
def main():
    ...
    save_point_cloud(f"vision3D/nube_3D/output/BM3_python.ply", disparity, colors) # Guarda la nube de puntos en un archivo PLY
    plt.imsave(f"vision3D/nube_3D/output/BM3_python.png", disparity, cmap='jet') # Guarda el mapa de calor de la imagen en base a la nube de puntos

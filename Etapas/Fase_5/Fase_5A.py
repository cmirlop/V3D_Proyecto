...
def aplicar_homografia(img, H, output_shape):
    """
    Aplica una homografía H a una imagen img sin usar OpenCV.
    img: imagen 2D (grayscale) o 3D (RGB)
    H: matriz 3x3
    output_shape: (alto, ancho) de la imagen de salida
    """
    H_inv = np.linalg.inv(H)

    height, width = output_shape
    coords = np.indices((height, width)).reshape(2, -1)  # (2, H*W)
    coords = np.vstack((coords[1], coords[0], np.ones((height * width))))  # (3, H*W)

    # Transformar coordenadas
    src_coords = H_inv @ coords
    src_coords /= src_coords[2]  # Normalizar
    ...
    if img.ndim == 2:
        ...
        warped = map_coordinates(img, [y_src, x_src], order=1, mode='reflect')
    else:
        # Imagen RGB
        warped = np.zeros((height, width, img.shape[2]), dtype=img.dtype)
        for c in range(img.shape[2]):
            warped[..., c] = map_coordinates(img[..., c], [y_src, x_src], order=1, mode='reflect')
